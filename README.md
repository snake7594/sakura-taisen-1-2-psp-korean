...
| 타이틀 화면 | 사쿠라1 — 간판(공연 / 제국화격단), 메뉴 라벨 5개(게임 시작 / START 버튼을 누르세요 / 사쿠라대전 2로 ×2 / 원화 표기)<br>사쿠라2 — 게임 시작 / 사쿠라대전 1로 / 일러스트 감상 |
...
```bash
python -c "import sys;sys.path.insert(0,'tools');import tsvio;c,r=tsvio.read('text/sakura1_adv.tsv');print(max(len(l) for x in r for l in (x['ja'] or '').split(chr(92)+'n')))"
```

저는 화면에 자를 그려 재다가 21자로 잘못 알았습니다. 얼굴 그림이 붙은
대사창은 글자 시작이 오른쪽으로 밀려 18자만 들어갑니다.
...
`[u16 w][u16 h][u16 fmt][u16 뱅크][u32 off][u32 size]`. fmt 하위 니블이
색 형식(1→4bpp, 4→8bpp, 5→16bpp), 상위 니블이 LZSS 압축 여부.

**여기서 두 번 사고를 냈습니다. 고치실 분은 꼭 읽어 주세요.**

- **fmt 하위 니블을 반드시 확인하세요.** `TL_NEW.SPR` 을 16bpp 로 착각해
  RGB 로 읽고 u16 으로 되썼더니, 8bpp 인 그림에 한 픽셀당 2바이트가 들어가
  게임이 그걸 인덱스로 읽어 무지개 잡음이 됐습니다.
- **8bpp 는 장마다 팔레트 뱅크가 다릅니다.** 엔트리의 네 번째 u16 이
  2048색 팔레트 안 시작 위치입니다 (0, 256, 512 …). 그 뱅크 안에서
  인덱스를 골라야 합니다.
- **16bpp 는 ABGR1555 (R 이 하위)** 이고, 읽기·쓰기 짝은
  `spr_write.unpack16` + `spr_write.pack_px` 입니다 (둘 다 빅엔디언).
  `spr.decode_pixels` 의 16bpp 갈래는 리틀엔디언이라 짝이 안 맞습니다.
  왕복이 맞는지 먼저 확인하세요 — `pack_px(unpack16(원본),16) == 원본`.
- **손대지 않을 픽셀은 RGB 로 갔다 오지 마세요.** 5비트로 반올림되면서
  그림 전체가 미세하게 변합니다. 고칠 상자 안만 건드리세요.
