...
# 낱장 찾기 메모
#   폭을 16바이트(한 글자)씩, 시작점도 16바이트씩 옮겨 가며 32행 창을 훑어
#   위아래 2행 잉크가 0 인 자리만 남기면 낱장 후보가 나온다.
#   다만 가장자리 검사로는 **가로 회전을 못 가린다** — 폭이 W 일 때 16바이트를
#   밀면 글자가 한 칸 돌아갈 뿐 가장자리는 그대로다. 그래서 마지막에는
#   반드시 렌더해서 글자 순서를 눈으로 확인해야 한다.
#   대화상자 5개는 0x03900 부터 정확히 이어 붙어 있다(W*32 씩). 다만 그 뒤로는
#   높이가 32행이 아닌 낱장이 섞여 있어 사슬 계산이 이어지지 않는다.
