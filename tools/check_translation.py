...
#   사쿠라 1 대사창 : **18자**. 전에 21자로 알고 있었는데 틀렸다 —
#              얼굴 그림이 붙은 대사창은 글자 시작이 오른쪽으로 밀려 18자만 든다.
#              실기 사진에서 19자가 접혀 마지막 줄이 화면 밖으로 나갔다.
#              **원문이 가장 믿을 만한 근거다** — 일본어 대본도 최대 18자다.
#   사쿠라 1 전투창 : 17자 (원문 최대 17자)
#   사쿠라 2 : 14자 (자가 14 / 14 / 나머지 로 갈림). 원문도 99%가 14자 이하다.
#              15번째 글자가 줄바꿈에서 사라지는 현상이 있어 14자를 넘기면 안 된다.
LIMITS   = {'sakura1_slg': 17, 'sakura1': 18, 'sakura2': 14}
